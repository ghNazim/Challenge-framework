...
camera.up.set(0, 0, 1);
camera.position.set(0, cylinderRadius * 2, cylinderHeight * 1.25);
camera.lookAt(0, 0, 0);
...
function callWithStep(step) {
  switch (step) {
    ...
    case 3:
      ...
      break;
    case 4:
      wrapper.style.transform = "translateX(-15%)";
      revealVolumeFormula();
...
function drawLabels3(overlay) {
  ...
  const xx = (Math.PI * cylinderRadius) / 2,
    yy = cylinderRadius / 2,
    zz = cylinderHeight;
    s = cylinderRadius * Math.sin(anglePerSector / 2);
  const labelAnchors3d = [
    new THREE.Vector3(xx, yy + 0.2, 0),
    new THREE.Vector3(-xx, yy + 0.2, 0),
    new THREE.Vector3(xx + 0.1, yy, 0),
    new THREE.Vector3(xx + s + 0.1, -yy, 0),
    new THREE.Vector3(xx + s + 0.05, -yy, 0),
    new THREE.Vector3(xx + s + 0.05, -yy, zz),
    new THREE.Vector3(0, yy + 1, 0),
    new THREE.Vector3(xx + 0.22 + s / 2, .2, 0),
    new THREE.Vector3(xx + 0.18 + s , -yy, zz / 2),
  ];
...
const resizeObserver = new ResizeObserver((entries) => {
  ...
  // Update camera properties for OrthographicCamera
  const newAspectRatio = width / height;
  const d = 2.5; // Your existing camera distance factor

  camera.left = -d * newAspectRatio;
  camera.right = d * newAspectRatio;
  // Keep top and bottom as they are if you don't want vertical scaling
  // camera.top = d;
  // camera.bottom = -d;

  // Or if you want to adjust the view to fit the new aspect ratio fully:
  camera.top = d + 0.6; // Keep your custom vertical offset
  camera.bottom = -d;

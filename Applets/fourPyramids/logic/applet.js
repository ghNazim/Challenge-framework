...
function createPyramidFrustum(baseType, height, s) {
  height = height*s;
  const bottomRadius =1,topRadius = bottomRadius * (1-s);
...
function createRectangularFrustum(
  height,s
) {
  const widthBottom = 2,depthBottom =1;
  const widthTop = (1-s)*widthBottom, depthTop = (1-s)*depthBottom;

...
const bottomMidQuestion = document.getElementById("bottom-mid-question");
...
let optype = "subtraction",
  checkExpression = null,
  findNextAvailableBox = null;
function setOptype() {
  const isSubtraction = currentProblemIndex % 2 === 0;
  optype = isSubtraction ? "subtraction" : "addition";
  checkExpression = isSubtraction
    ? checkExpressionSubtraction
    : checkExpressionAddition;
  findNextAvailableBox = isSubtraction
    ? findNextAvailableBoxSubtraction
    : findNextAvailableBoxAddition;
}
...
const problems = [
  ...
  questionData.problem_4,
];
const twoDigitData = [[0], [0, 4], null, [4]];
function setUpTwoDigit(indexArr) {
  document
    .querySelectorAll(".two-digit")
    .forEach((box) => box.classList.remove("two-digit"));
...
function initiateProblem(problemIndex) {
  ...
  if(optype === "subtraction") {
    diagramImage.src = `assets/problem_${problemIndex + 1}.png`;
  }
  fillContextWithTag(-1, "stage_1_prompt_" + optype); // Use common tag
  populateNumberPad();
  ...
  bottomMidQuestion.innerHTML = `<p>${problem.problem_mid}</p>`;
...
function loadMCQ(mcqData) {
  if (currentStage === 2) {
    fillContextWithTag(-1, "initial_prompt_" + optype); // Use common tag
  } else if (currentStage === 3) {
    // When moving to MCQ 2, show the success message from MCQ 1
    fillContextWithTag(-1, "mcq_2_initial_" + optype); // Use common tag
...
function checkAnswer(selectedIndex, correctIndex, optionElement) {
  ...
  const feedbackTag = `mcq_${stageNum}_feedback_${feedbackType}_${optype}`;
...
function loadExpressionStage() {
  fillContextWithTag(-1, "expression_prompt_" + optype); // Use common tag
  hintButton.style.display = "none";
...
function checkExpressionAddition() {
  const digitBoxes = document.querySelectorAll(".digit-box");
...
function checkExpressionSubtraction() {
  const digitBoxes = document.querySelectorAll(".digit-box");
  const problem = problems[currentProblemIndex];
  const correctAnswer = problem.correct_answer;
...
function findNextAvailableBoxAddition(startIndex) {
  const problem = problems[currentProblemIndex];
  const correctAnswer = problem.correct_answer;
...
function findNextAvailableBoxSubtraction(startIndex) {
  const problem = problems[currentProblemIndex];
  const correctAnswer = problem.correct_answer;
  const len = expression_array.length;
  for (let i = startIndex; i < len; i++) {
...
function handleHintClick() {
  hintButton.style.display = "none";
  if (currentStage === 2) {
    fillContextWithTag(-1, "hint_1_" + optype, "blue"); // Use common tag
  } else if (currentStage === 3) {
    fillContextWithTag(-1, "hint_2_" + optype, "blue"); // Use common tag
  }
}
...
function updateQuestionStage3() {
  const highlights = problems[currentProblemIndex].highlights;
  const highlightsTop = highlights
    .filter((highlight) => highlight.place === "top")
    .map((highlight) => highlight.text);
  const highlightsBottom = highlights
    .filter((highlight) => highlight.place === "bottom")
    .map((highlight) => highlight.text);
  const updatedQuestionTop = highlightLastOccurance(
    highlightsTop,
    problems[currentProblemIndex].problem_statement
  );
  questionSection.innerHTML = `<p>${updatedQuestionTop}</p>`;
  const updatedQuestionBottom = highlightLastOccurance(
    highlightsBottom,
    problems[currentProblemIndex].problem_mid
  );
  bottomMidQuestion.innerHTML = `<p>${updatedQuestionBottom}</p>`;
}
function updateQuestionCorrectStage3() {
  ...
  const highlights = problems[currentProblemIndex].highlights;
  const toHighlight = highlights[correct_answer];
  if (toHighlight.place === "top") {
    const updatedQuestionTop = highlightLastOccurance(
      [toHighlight.text],
      problems[currentProblemIndex].problem_statement
    );
...
function updateQuestionHintStage4() {
  ...
  const updatedQuestion = highlightLastOccurance(
    [expressionArray[0], expressionArray[2]],
    problems[currentProblemIndex].problem_statement
  );

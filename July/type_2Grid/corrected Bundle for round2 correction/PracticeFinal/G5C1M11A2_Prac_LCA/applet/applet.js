...
const row7plusSigns = document.querySelectorAll(".row-7.plus-sign");
optionsTitle.textContent = data.pick;
document.querySelector(".carry-over-text").textContent = data.carry_over;
// --- Text Population ---
...
function populateSumContainer(index) {
  ...
  currentQuestion.forEach((num, i) => {
    ...
    // The last number gets the 'operation' class for styling.
    ...
    // The first number has no plus sign, subsequent ones do.
    if (i > 0) {
      div.textContent = `+ ${num}`;
    } else {
      div.textContent = num;
...
function setupStage3() {
  ...
  row7plusSigns.forEach((box) => (box.style.visibility = "visible"));
...
function handleStage2OptionClick(event) {
  ...
  if (activeButton.textContent.trim() === clickedButton.textContent.trim()) {
    ...
    if (currentActiveBoxIndex < questions[questionIndex].length * 5) {
      setActiveBox(currentActiveBoxIndex);
      ...
      updateOptionsFooter("excellent2", "green");
    }
  } else {
...
function setActiveResultBox(index, isFinal = false) {
  ...
  if (isFinal) {
    ...
    row7plusSigns.forEach((box) => box.style.color = "white");
...
function populateGrid(index) {
  ...
  currentQuestion.forEach((num, i) => {
    if (i < numberLabels.length) {
      // Add a plus sign before all numbers except the first one
      if (i > 0) {
...
function getJumbledPlaceValues(index) {
  ...
  // Add this new part to ensure the number of options is a multiple of 3
  while (allPlaceValues.length % 3 !== 0) {
    // Add a random incorrect number.
    // This adds a random two-digit number to avoid confusion with single-digit answers.
    allPlaceValues.push(Math.floor(10 + Math.random() * 90));
  }

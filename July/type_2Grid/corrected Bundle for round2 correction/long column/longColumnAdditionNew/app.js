...
function showFtue(element, offsetDown = 0) {
  if (!element) return;
  ...
  else if(offsetDown==2){
    handFtue.classList.add("hand-offset2");
  }
  else{
    handFtue.classList.remove("hand-offset");
    handFtue.classList.remove("hand-offset2");
  }
...
function triggerMCQoverlay(show) {
  if(show){
    document.querySelector(".main-container").classList.add("no-backdrop");
  }
  else{
    document.querySelector(".main-container").classList.remove("no-backdrop");
  }
  const display = show ? "flex" : "none";

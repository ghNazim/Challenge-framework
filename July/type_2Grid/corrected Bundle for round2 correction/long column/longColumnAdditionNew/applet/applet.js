...
function getDataFromQuestion() {
  ...
  templateDataMCQ = {
    ...
    oOverflowStr: oOverflow ? " + 10" : "",
    tOverflowStr: tOverflow ? " + 100" : "",
    hOverflowStr: hOverflow ? " + 1000" : "",
    thOverflowStr: thOverflow ? " + 10000" : "",
  };
}
...
function afterMcq2() {
  ...
  loadMcqIn("ones_split", () => {
    ...
    highlightBox.onclick = moveCarry;
    highlightBox.classList.add("clickable");
    showTextWithTag("ones_carryover2");
    splitOverlay(false);
    showFtue(onesResultCell, 2);
  });
}
function moveCarry() {
  borderRed(false);
...
function animateCarry(q1, q2, tag) {
  ...
  // highlightBox.classList.add("no-animation")
  ...
  setTimeout(() => {
    ...
    highlightColumnBorder(tagToColumn[tag]);
    borderGreen(true);
    // highlightBox.classList.remove("no-animation")
  }, 500);
}
...
function afterTensMcq2() {
  ...
  loadMcqIn("tens_split", () => {
    ...
    tensResultCell.classList.add("clickable");
    showFtue(tensResultCell, 2);
    highlightBox.onclick = moveTensCarry;
    highlightBox.classList.add("clickable");
...
function afterHundredsMcq2() {
  ...
  loadMcqIn("hundreds_split", () => {
    ...
    hundredsResultCell.classList.add("clickable");
    showFtue(hundredsResultCell, 2);
    highlightBox.onclick = moveHundredsCarry;
    highlightBox.classList.add("clickable");
...
function afterThousandsMcq2() {
  ...
  loadMcqIn("thousands_split", () => {
    ...
    thousandsResultCell.classList.add("clickable");
    showFtue(thousandsResultCell, 2);
    highlightBox.onclick = moveThousandsCarry;
    highlightBox.classList.add("clickable");
...
function initiate() {
  ...
  fillNumbers();
  fillHeaders();
  // Get all carry-over rows
  const carryRows = document.querySelectorAll(".carry-over");
  // Check if it's the first two questions
  if (questionIndex < 2) {
    // Add the 'hidden' class to hide them
    carryRows.forEach((row) => row.classList.add("undisplayed"));
  } else {
    // Remove the 'hidden' class to show them for other questions
    carryRows.forEach((row) => row.classList.remove("undisplayed"));
  }
  stage0();
}

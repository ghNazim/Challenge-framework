...
function showDragFtue(sliderElement) {
  if (!sliderElement) return;
  const handFtue = document.getElementById("hand-ftue");
  const handFtue2 = document.getElementById("hand-ftue2");
  const rect = sliderElement.getBoundingClientRect();

  if (sliderElement.id.includes("horizontal")) {
    handFtue.style.top = `${rect.top + rect.height / 2}px`;
    handFtue.style.left = `${rect.left}px`;
    handFtue.classList.add("hand-dragging", "horizontal-drag");
  } else {
    handFtue2.style.top = `${rect.top + rect.height}px`;
    handFtue2.style.left = `${rect.left + rect.width / 2}px`;
    handFtue2.classList.add("hand-dragging", "vertical-drag");
  }
}

function hideDragFtue(n = 0) {
  if (n == 0) {
    const handFtue = document.getElementById("hand-ftue");
    handFtue.classList.remove(
      "hand-dragging",
      "horizontal-drag",
      "vertical-drag"
    );

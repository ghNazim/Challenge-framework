...
function addMinusSign(targetRow) {
  ...
  minusBlock.style.left = `${
    targetRect.left - containerRect.left - minusBlock.offsetWidth - 20
  }px`;
  ...
  minusNumber.style.left = `${
    targetNumRect.left - containerRect.left - minusNumber.offsetWidth - 20
  }px`;
}
...
async function handleBorrowAnimation() {
  ...
  await new Promise(resolve => {
        ...
        animateCloneToTarget(sourceTenRod, tempTarget,
            ...
            async () => {
                ...
                if (overflowUnits.length > 0) {
                    ...
                    minuend.ones.blockContainer.classList.add("is-overflowing", "unit-block-overflow");
                }
                resolve();
            }
        );

...
function handleStepperClick(event) {
  ...
  if (currentStep === 4 && row.id === 1) {
    ...
    row.updateBlockVisuals(row.count, true);
...
function advanceStep() {
  ...
  switch (currentStep) {
    ...
    case 6: handleTakeAway(); break;
    case 7: handleCollectRemaining(); break;
    case 8: setupStep5(); break;
  }
}
...
function setupStep3() {
  ...
  createRow();
  addEqualsLine();
  nextButton.textContent = texts.buttons.take_away;
  nextButton.disabled = false;
  showFtue(nextButton, true);
}

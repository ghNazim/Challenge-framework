...
function setAnsDisplay() {
  // Pad each number to ensure 3-digit display like '023'
  const h = current_number[2][0];
  const t = current_number[2][1];
  const u = current_number[2][2];
  ansDisplay.textContent = `${h}${t}${u}`;
}
...
function initializeBoard() {
  ...
  if (!problem) {
    updateInstructionText("finished");
    document.querySelector(".problem-statement h1").textContent =
      "All Questions Done!";

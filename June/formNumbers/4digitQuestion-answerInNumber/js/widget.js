...
function addBlocksMultiple(tag, n, visible = true) {
  for (let i = 0; i < n; i++) {
    addBlock(tag, visible);
  }
}
...
/**
 * Updates all widgets in the box container to visually represent a given 4-digit number.
 * It clears the existing state and then adds the correct number of blocks to each widget.
 *
 * @param {number} number - The 4-digit number to display.
 */
function updateAllWidgets(number) {
  // 1. Reset all widgets to their initial empty state.
  resetContainers();

  // 2. Convert the number to a 4-digit string (e.g., 123 becomes "0123").
  const numberString = String(number).padStart(4, '0');
  const digits = numberString.split('').map(Number); // [thousands, hundreds, tens, units]

  // 3. Define the order of place values corresponding to the digits array.
  const placeValues = ['thousand', 'hundred', 'ten', 'unit'];

  // 4. Iterate through each digit and update the corresponding widget.
  digits.forEach((digitValue, index) => {
    const tag = placeValues[index];

    // Only update if the digit is greater than 0.
    if (digitValue > 0) {
      // Update the internal count for the current place value.
      countObject[tag] = digitValue;

      // Update the number displayed on the tab above the widget.
      updateNumberTab(tag, digitValue);

      // Add the correct number of visual blocks to the widget's container.
      addBlocksMultiple(tag, digitValue, true);
    }
  });
}

...
const [ttLeftDir, tLeftDir, hLeftDir, tenLeftDir, unitLeftDir] =
...
const countObject = {
  ...
  squareContainerObject = {
    ...
    hundredThousand: htSquaresContainer,
...
async function animateOverflow(tag) {
  ...
  // if (bigTT) {
  //   document.querySelector("#tenThousand-widget").classList.add("bigTenThousand");
  // }
...
ttLeftDir.addEventListener("click", () => {
  setTimeout(() => {
    confettiBurst();
    playSound("congrats");
    showControlButtons("hundredThousand", false);
    plusBtnObject.hundredThousand.classList.remove("pulse-highlight");
  }, 500);
});
...
function firstClickTTplus() {
  console.log(countObject.tenThousand);
  
  if (countObject.tenThousand < 6) {
    return;
  }
  ttPlus.removeEventListener("click", firstClickTTplus);
  document
    .querySelector("#tenThousand-widget .squares-container")
    .classList.remove("bigTenThousand");
}
ttPlus.addEventListener("click", firstClickTTplus);

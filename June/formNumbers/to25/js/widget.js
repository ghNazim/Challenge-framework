...
function updateUnitTexts(n = unitCount) {
  ...
  unitTextDisplay.textContent =
    n + (n === 1 ? tags.one : tags.ones);
}
function updateTenTexts(n = tenCount) {
  tenNumberTab.textContent = n;
  tenTextDisplay.textContent =
    n + (n === 1 ? tags.ten : tags.tens);
}
function updateTextOfTensAndUnits() {
  unitTextDisplay.textContent =
    unitCount + (unitCount === 1 ? tags.one : tags.ones);
  tenTextDisplay.textContent =
    tenCount + (tenCount === 1 ? tags.ten : tags.tens);
}
unitsPlus.addEventListener("click", () => {
  ...
  if (unitCount == 10) {
    ...
    showNumberText(tenCount * 10 + unitCount, false);
  } else {
